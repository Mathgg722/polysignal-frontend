...
export default function HighConfidence() {
  ...
  useEffect(() => {
    ...
    fetch(`${BASE}/markets?max_days=365`)
      ...
      .then(data => {
        const filtered = data
          .filter(m => Math.max(m.yes_price, m.no_price) >= 80)
          .sort((a, b) => Math.max(b.yes_price, b.no_price) - Math.max(a.yes_price, a.no_price))
        setMarkets(filtered)
      })
      .catch(() => setMarkets([]))
            ...
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center' }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 600, color: urgencyColor(m.days_to_close) }}>
                ...
                {m.days_to_close <= 1
                  ? 'fecha hoje'
                  : m.days_to_close <= 7
                  ? `fecha em ${m.days_to_close}d`
                  : `${m.days_to_close} dias`}
              </span>
              {vol24 && (
                <span style={{ fontSize: 11, color: 'rgba(255,255,255,0.3)' }}>{vol24}</span>
              )}
              <a
